...
if (isset($_SERVER['CONTENT_LENGTH'])) {// request has a body : delay your processing into a separate worker
    ...
    file_put_contents($folder . '/' . $u . '.callback.log', file_get_contents('php://input'));
    file_put_contents('callback.processing.time.log', "\n" . $u . ':' . round((microtime(true) - $started)*1000).'ms', 8);
}
